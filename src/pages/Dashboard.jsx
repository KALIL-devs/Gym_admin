...
// ✅ NEW IMPORT: Import the PaymentHistory component
...
// ✅ NEW IMPORT: Icon for Payment History
...
function Dashboard({ user, setLoggedInUser }) {
  ...
  const fetchClients = async () => {
    try {
      // Assuming you have a standard fetch setup that includes authorization headers
  ...
  useEffect(() => {
    // Only fetch if the user is logged in (i.e., 'user' prop is available)
  ...
  // --- UPDATED FILTERING LOGIC (Unchanged, but kept for context) ---
  const filteredClients = (Array.isArray(clients) ? clients : [])
    .filter((c) => {
      ...
      // Check if search term matches Name OR Roll No.
      ...
      const isSearchMatch = isNameMatch || isRollNoMatch; 
    ...
    <div className="flex min-h-screen bg-dark-bg text-dark-text dark:bg-light-bg dark:text-light-text">
      ...
      <aside className="fixed top-0 left-0 z-50 h-screen w-64 hidden sm:flex flex-col bg-dark-card dark:bg-light-card shadow-xl">
        ...
        <nav className="flex-1 px-4 space-y-3 overflow-y-auto">
          ...
          <button
            className={`flex items-center gap-3 w-full p-3 rounded-lg hover:bg-dark-accent hover:text-white dark:hover:bg-light-accent ${activeModal === 'payment-log' ? 'bg-dark-accent text-white dark:bg-light-accent' : ''}`}
            onClick={() => setActiveModal('payment-log')} 
          >
      ...
      <main className="flex-1 flex flex-col sm:pl-64">
        ...
        <header className="flex justify-between items-center px-6 py-4 shadow-md bg-dark-card dark:bg-light-card">
          <h1 className="text-xl font-semibold text-dark-accent dark:text-light-accent">
            {activeModal === 'renew-membership' 
                ...
                ? 'Renewal History'
                // ✅ NEW: Title for Payment Log
                : activeModal === 'payment-log'
                ? 'Payment History'
        ...
        {/* ✅ NEW: Conditional rendering for Payment History */}
        {activeModal === 'payment-log' && (
            <div className="flex-1 overflow-auto">
                <PaymentHistory />
            </div>
